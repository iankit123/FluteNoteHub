...
const Home: React.FC = () => {
  ...
  >({
    ...
    select: (data) =>
      data.filter((tutorial) => user && tutorial.authorId === user.id),
  ...
  const getNotesAndTutorials = (): Array<Tutorial | Note> => {
    ...
    if (tutorials && tutorials.length > 0) {
      const educationalTutorials = tutorials.filter(
        (tutorial) =>
          tutorial.source !== "youtube" ||
          (tutorial.source === "youtube" &&
            !tutorial.title.toLowerCase().includes("music")),
      );
  ...
  const getMusicContent = (): TutorialWithTags[] => {
    ...
    return tutorials.filter((tutorial) => tutorial.source === "youtube");
  };
      ...
      <main className="container mx-auto px-4 py-6 wave-bg">
        ...
        <div className="mb-8">
          ...
          <p className="text-dark-slate/70 mt-2">
            Continue your flute journey with new tutorials and personal notes.
          </p>
        </div>
            ...
            {isLoading ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-24">
                {[1, 2, 3, 4, 5, 6].map((i) => (
                  <div
                    key={i}
                    className="bg-white rounded-xl shadow-card h-80 animate-pulse"
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-24">
                    ...
                    >
                      <div className="bg-white rounded-xl shadow-card overflow-hidden border-l-4 border-turmeric-yellow">
                        <div className="p-6">
                          ...
                          <h3 className="font-poppins font-semibold text-lg text-dark-slate mb-2">
                            {note.title}
                          </h3>
                          <div className="text-dark-slate/70 text-sm mb-4 overflow-hidden h-24">
                            ...
                            {note.content.split("\n").length > 4 && (
                              <p className="text-royal-purple font-medium text-xs mt-1">
                                + {note.content.split("\n").length - 4} more
                                lines
                              </p>
                            )}
                          ...
                          <div className="flex justify-between items-center pt-2 border-t border-gray-100">
                            <div className="flex items-center space-x-1">
                              <svg
                                ...
                                className="h-5 w-5 text-royal-purple"
                                viewBox="0 0 20 20"
                                fill="currentColor"
                              >
                                <path
                ...
                {(!notes || notes.length === 0) &&
                  (!tutorials ||
                    tutorials.filter(
                      (t) =>
                        t.category === "learning" || t.category === undefined,
                    ).length === 0) && (
                    <div className="col-span-3 flex flex-col items-center justify-center bg-white/70 rounded-xl shadow-sm p-10 text-center mb-24">
                      ...
                      <h2 className="font-poppins font-semibold text-xl text-dark-slate mb-2">
                        No learning materials yet
                      </h2>
                      <p className="text-dark-slate/70 mb-6 max-w-md">
                        Start adding personal notes or save flute tutorials to
          ...
          <TabsContent value="good-music" className="mt-2">
            {isLoading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-24">
                ...
                {(!tutorials ||
                  tutorials.filter((t) => t.category === "music").length ===
                    0) && (
                  <div className="col-span-3 flex flex-col items-center justify-center bg-white/70 rounded-xl shadow-sm p-10 text-center mb-24">
                    ...
                    <p className="text-dark-slate/70 mb-6 max-w-md">
                      Add YouTube links to your favorite flute music to build a
                      collection of inspiring performances.
                    </p>
                    <div className="flex flex-wrap gap-3 justify-center">
        ...
        <div className="fixed bottom-24 right-6 z-20 md:bottom-8 flex flex-col gap-3">
          ...
          <AddNoteDialog
            initialCategory={activeTab === "good-music" ? "music" : "learning"}
          />
        </div>
      </main>
    </>
